...
intptr_t JASP_GameSupport::syscall(intptr_t cmd, ...) {
    ...
    if (cmd != G_PRINT)
        QMMLOG(QMM_LOG_TRACE, "QMM") << "JASP_GameSupport::syscall(" << EngMsgName(cmd) << "(" << cmd << ")) reutrning " << ret << "\n";
...
void* JASP_GameSupport::Entry(void* import, void*, APIType) {
    QMMLOG(QMM_LOG_DEBUG, "QMM") << "JASP_GameSupport::Entry(" << import << ") called\n";
    ...
    QMMLOG(QMM_LOG_DEBUG, "QMM") << "JASP_GameSupport::Entry(" << import << ") returning " << &qmm_export << "\n";
