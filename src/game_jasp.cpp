...
static game_import_t qmm_import = {
		...
		GEN_IMPORT(SetActiveSubBSP, G_SETACTIVESUBBSP),
...
static game_export_t qmm_export = {
	...
	GEN_EXPORT(GameSpawnRMGEntity, GAME_GAMESPAWNRMGENTITY),
...
intptr_t JASP_syscall(intptr_t cmd, ...) {
	...
	switch (cmd) {
		...
		ROUTE_IMPORT(SetActiveSubBSP, G_SETACTIVESUBBSP);
...
intptr_t JASP_vmMain(intptr_t cmd, ...) {
	...
	switch (cmd) {
		...
		ROUTE_EXPORT(GameSpawnRMGEntity, GAME_GAMESPAWNRMGENTITY);
...
const char* JASP_mod_msg_names(intptr_t cmd) {
	switch(cmd) {
		...
		GEN_CASE(GAME_GAMESPAWNRMGENTITY);
