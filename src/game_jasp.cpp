...
struct JASP_GameSupport : public GameSupport {
    ...
    virtual const char* GameName() { return "Jedi Knight: Jedi Academy (SP)"; }
...
bool JASP_GameSupport::AutoDetect(APIType engineapi) {
...
intptr_t JASP_GameSupport::syscall(intptr_t cmd, ...) {
...
#ifdef _DEBUG
    if (cmd != G_PRINT)
        LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("JASP_GameSupport::syscall({} {}) called\n", EngMsgName(cmd), cmd);
#endif
    ...
    switch (cmd) {
        ...
        ROUTE_IMPORT_3(G2API_RagPCJGradientSpeed, G_G2API_RAGPCJGRADIENTSPEED, CGhoul2Info_v&, const char*, const float);
...
#ifdef _DEBUG
    if (cmd != G_PRINT)
        LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("JASP_GameSupport::syscall({} {}) returning {}\n", EngMsgName(cmd), cmd, ret);
#endif
...
#ifdef _DEBUG
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("JASP_GameSupport::vmMain({} {}) called\n", ModMsgName(cmd), cmd);
#endif
...
#ifdef _DEBUG
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("JASP_GameSupport::vmMain({} {}) returning {}\n", ModMsgName(cmd), cmd, ret);
#endif
...
void* JASP_GameSupport::Entry(void* import, void*, APIType) {
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("JASP_GameSupport::Entry({}) called\n", import);
    ...
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("JASP_GameSupport::Entry({}) returning {}\n", import, fmt::ptr(&qmm_export));
...
void JASP_GameSupport::ModUnload() {
    orig_export = nullptr;
}


const char* JASP_GameSupport::EngMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(G_PRINTF);
        GEN_CASE(G_WRITECAM);
...
const char* JASP_GameSupport::ModMsgName(intptr_t cmd) {
    switch (cmd) {
...
// a copy of the original import struct that comes from the game engine
game_import_t JASP_GameSupport::orig_import = {};


// a copy of the original export struct pointer that comes from the mod
game_export_t* JASP_GameSupport::orig_export = nullptr;


// struct with lambdas that call QMM's syscall function. this is given to the mod
game_import_t JASP_GameSupport::qmm_import = {
        ...
        GEN_IMPORT_1_F(WE_IsOutsideCausingPain, G_WE_ISOUTSIDECAUSINGPAIN, vec3_t),
        GEN_IMPORT_0_F(WE_GetChanceOfSaberFizz, G_WE_GETCHANCEOFSABERFIZZ),
...
// track entstrings for our G_GET_ENTITY_TOKEN syscall
std::map<intptr_t, std::vector<std::string>> JASP_GameSupport::subbsp_entity_tokens;  // -1 = main map entities
std::map<intptr_t, size_t> JASP_GameSupport::token_counter;
