...
GEN_GAME_QMM_MSGS(MOHBT);
GEN_GAME_EXTS(MOHBT);

GEN_GAME_FUNCS(MOHBT);
...
static void* MOHBT_Entry(void* import, void*, api_engine) {
    ...
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("MOHBT_Entry({}) returning {}\n", import, (void*)&qmm_export);
...
static void MOHBT_ModUnload() {
    orig_export = nullptr;
}


static const char* MOHBT_EngMsgNames(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(G_PRINTF);
        GEN_CASE(G_DPRINTF);
        GEN_CASE(G_DPRINTF2);
...
static const char* MOHBT_ModMsgNames(intptr_t cmd) {
    switch (cmd) {
