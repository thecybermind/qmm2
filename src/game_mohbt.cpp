...
struct MOHBT_GameSupport : public GameSupport {
    ...
    virtual const char* DefaultModDir() { return "maintt"; }
    virtual const char* GameName() { return "Medal of Honor: Breakthrough"; }
...
bool MOHBT_GameSupport::AutoDetect(APIType engineapi) {
...
intptr_t MOHBT_GameSupport::syscall(intptr_t cmd, ...) {
    QMM_GET_SYSCALL_ARGS();

#ifdef _DEBUG
    if (cmd != G_PRINT)
        LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("MOHBT_GameSupport::syscall({} {}) called\n", EngMsgName(cmd), cmd);
#endif
    ...
    case G_FS_FOPEN_FILE: {
        // not really sure what's up with fileHandle_t-based G_FS_ functions in MOHBT, but we are just
...
#ifdef _DEBUG
    if (cmd != G_PRINT)
        LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("MOHBT_GameSupport::syscall({} {}) returning {}\n", EngMsgName(cmd), cmd, ret);
#endif
...
intptr_t MOHBT_GameSupport::vmMain(intptr_t cmd, ...) {
    QMM_GET_VMMAIN_ARGS();

#ifdef _DEBUG
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("MOHBT_GameSupport::vmMain({} {}) called\n", ModMsgName(cmd), cmd);
#endif
...
#ifdef _DEBUG
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("MOHBT_GameSupport::vmMain({} {}) returning {}\n", ModMsgName(cmd), cmd, ret);
#endif
...
void* MOHBT_GameSupport::Entry(void* import, void*, APIType) {
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("MOHBT_GameSupport::Entry({}) called\n", import);
    ...
    LOG(QMM_LOG_DEBUG, "QMM") << fmt::format("MOHBT_GameSupport::Entry({}) returning {}\n", import, fmt::ptr(&qmm_export));
...
void MOHBT_GameSupport::ModUnload() {
    orig_export = nullptr;
}


const char* MOHBT_GameSupport::EngMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(G_PRINTF);
        GEN_CASE(G_DPRINTF);
        GEN_CASE(G_DPRINTF2);
...
const char* MOHBT_GameSupport::ModMsgName(intptr_t cmd) {
    switch (cmd) {
...
void MOHBT_GameSupport::update_exports() {
    if (!orig_export)
        return;
...
game_import_t MOHBT_GameSupport::orig_import;


game_export_t* MOHBT_GameSupport::orig_export = nullptr;


game_import_t MOHBT_GameSupport::qmm_import = {
...
std::vector<std::string> MOHBT_GameSupport::entity_tokens;
...
void MOHBT_GameSupport::DebugCircle(float* arg0, float arg1, float arg2, float arg3, float arg4, float arg5, qboolean arg6) {
    cgame.is_from_QMM = true;
...
// struct with lambdas that call QMM's vmMain function. this is given to the game engine
game_export_t MOHBT_GameSupport::qmm_export = {
    ...
    MOHBT_GameSupport::SpawnEntities,
