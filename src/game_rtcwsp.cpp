...
intptr_t RTCWSP_syscall(intptr_t cmd, ...) {
    ...
    if (cmd != G_PRINT)
        LOG(QMM_LOG_TRACE, "QMM") << fmt::format("RTCWSP_syscall({} {}) called\n", RTCWSP_eng_msg_names(cmd), cmd);
    ...
    if (cmd != G_PRINT)
        LOG(QMM_LOG_TRACE, "QMM") << fmt::format("RTCWSP_syscall({} {}) returning {}\n", RTCWSP_eng_msg_names(cmd), cmd, ret);
...
void RTCWSP_dllEntry(eng_syscall_t syscall) {
