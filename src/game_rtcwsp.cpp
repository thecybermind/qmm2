...
static const char* RTCWSP_EngMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(G_PRINT);
        GEN_CASE(G_ERROR);
        GEN_CASE(G_ENDGAME);
        ...
        GEN_CASE(BOTLIB_AAS_GETROUTEFIRSTVISPOS);
...
static const char* RTCWSP_ModMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(GAME_INIT);
