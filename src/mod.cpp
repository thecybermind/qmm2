...
bool Mod::Load(std::string file) {
    ...
    if (str_striequal(ext, EXT_QVM) && QMM::game->DefaultQVMName()) {
...
bool Mod::InitDLL(std::string file, void* handle, APIType dll_api) {
    ...
    case QMM_API_GETMODULEAPI: {
        ...
        QMMLOG(QMM_LOG_ERROR, "QMM") << "Mod::InitDLL(\"" << path_basename(file) << "\"): " << QMM::game->GameCode() << "_GameSupport::ModLoad returned false\n";
