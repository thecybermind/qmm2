...
struct JK2SP_GameSupport : public GameSupport {
    ...
    virtual const char* DefaultDLLName() { return "jk2game" MOD_DLL; }
    virtual const char* DefaultModDir() { return "."; }
    virtual const char* GameName() { return "Jedi Knight 2: Jedi Outcast (SP)"; }
...
bool JK2SP_GameSupport::AutoDetect(APIType engineapi) {
...
intptr_t JK2SP_GameSupport::syscall_args(intptr_t cmd, intptr_t* args) {
    if (cmd != G_PRINT)
        QMMLOG(QMM_LOG_TRACE, "QMM") << "JK2SP_GameSupport::syscall(" << EngMsgName(cmd) << "(" << cmd << ")) called\n";
    ...
    if (cmd != G_PRINT)
        QMMLOG(QMM_LOG_TRACE, "QMM") << "JK2SP_GameSupport::syscall(" << EngMsgName(cmd) << "(" << cmd << ")) reutrning " << ret << "\n";
...
intptr_t JK2SP_GameSupport::vmMain_args(intptr_t cmd, intptr_t* args) {
    QMMLOG(QMM_LOG_TRACE, "QMM") << "JK2SP_GameSupport::vmMain(" << ModMsgName(cmd) << "(" << cmd << ")) called\n";
    ...
    switch (cmd) {
        ...
        ROUTE_EXPORT_3(ValidateAnimRange, GAME_VALIDATEANIMRANGE, int, int, FLOAT_CAST);
    ...
    QMMLOG(QMM_LOG_TRACE, "QMM") << "JK2SP_GameSupport::vmMain(" << ModMsgName(cmd) << "(" << cmd << ")) returning " << ret << "\n";

    return ret;
}


void* JK2SP_GameSupport::Entry(void* import, void*, APIType) {
    QMMLOG(QMM_LOG_DEBUG, "QMM") << "JK2SP_GameSupport::Entry(" << import << ") called\n";
    ...
    QMMLOG(QMM_LOG_DEBUG, "QMM") << "JK2SP_GameSupport::Entry(" << import << ") returning " << &qmm_export << "\n";
...
void JK2SP_GameSupport::ModUnload(APIType) {
    orig_export = nullptr;
}


const char* JK2SP_GameSupport::EngMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(G_PRINTF);
        GEN_CASE(G_WRITECAM);
        GEN_CASE(G_ERROR);
...
const char* JK2SP_GameSupport::ModMsgName(intptr_t cmd) {
    switch (cmd) {
