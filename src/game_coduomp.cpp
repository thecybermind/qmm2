...
intptr_t CODUOMP_GameSupport::syscall(intptr_t cmd, ...) {
    ...
    if (cmd != G_PRINT)
        QMMLOG(QMM_LOG_TRACE, "QMM") << "CODUOMP_GameSupport::syscall(" << EngMsgName(cmd) << "(" << cmd << ")) called\n";
    ...
    if (cmd != G_PRINT)
        QMMLOG(QMM_LOG_TRACE, "QMM") << "CODUOMP_GameSupport::syscall(" << EngMsgName(cmd) << "(" << cmd << ")) reutrning " << ret << "\n";
...
void* CODUOMP_GameSupport::Entry(void* syscall, void*, APIType) {
    QMMLOG(QMM_LOG_DEBUG, "QMM") << "CODUOMP_GameSupport::Entry(" << syscall << ") called\n";

    // store original syscall from engine
    orig_syscall = (eng_syscall)syscall;

    QMMLOG(QMM_LOG_DEBUG, "QMM") << "CODUOMP_GameSupport::Entry(" << syscall << ") returning\n";
