...
bool JK2MP_GameSupport::AutoDetect(APIType engineapi) {
...
intptr_t JK2MP_GameSupport::syscall_args(intptr_t cmd, intptr_t* args) {
    ...
    switch (cmd) {
    // handle special cmds which QMM uses but JK2MP doesn't have an analogue for
    case G_ARGS: {
...
void JK2MP_GameSupport::ModUnload(APIType) {
    orig_vmMain = nullptr;
}


const char* JK2MP_GameSupport::EngMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(G_PRINT);
        GEN_CASE(G_ERROR);
        ...
        GEN_CASE(SP_REGISTER_SERVER_CMD);
...
const char* JK2MP_GameSupport::ModMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(GAME_INIT);
