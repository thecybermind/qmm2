...
#ifdef _DEBUG
    if (cmd != G_PRINT)
        LOG(QMM_LOG_TRACE, "QMM") << fmt::format("JK2MP_syscall({} {}) called\n", JK2MP_eng_msg_names(cmd), cmd);
#endif
...
#ifdef _DEBUG
    if (cmd != G_PRINT)
        LOG(QMM_LOG_TRACE, "QMM") << fmt::format("JK2MP_syscall({} {}) returning {}\n", JK2MP_eng_msg_names(cmd), cmd, ret);
#endif
...
void JK2MP_dllEntry(eng_syscall_t syscall) {
