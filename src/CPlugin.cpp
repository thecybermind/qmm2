...
int CPlugin::LoadQuery(std::string file) {
	...
	if (!x) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): DLL load failed for plugin: {}\n", file, dlerror()).c_str());
		return 0;
	}

	// find QMM_Query() or fail
	if ((this->QMM_Query = (plugin_query)this->dll.GetProc("QMM_Query")) == NULL) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Unable to find \"QMM_Query\" function in plugin\n", file).c_str());
		return 0;
	}

	// call QMM_Query() func to get the plugininfo
	(this->QMM_Query)(&(this->plugininfo), NULL);

	if (!this->plugininfo) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Plugininfo NULL for plugin", file).c_str());
		return 0;
	}
	...
	if (this->plugininfo->pifv_major > QMM_PIFV_MAJOR) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Plugin's major interface version ({}) is greater than QMM's ({}), suggest upgrading QMM.\n", file, this->plugininfo->pifv_major, QMM_PIFV_MAJOR).c_str());
		return 0;
	}
	// if the plugin's major version is lower, don't load and suggest to upgrade plugin
	if (this->plugininfo->pifv_major < QMM_PIFV_MAJOR) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Plugin's major interface version ({}) is less than QMM's ({}), suggest upgrading plugin.\n", file, this->plugininfo->pifv_major, QMM_PIFV_MAJOR).c_str());
		return 0;
	}
	...
	if (this->plugininfo->pifv_minor > QMM_PIFV_MINOR) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Plugin's minor interface version ({}) is greater than QMM's ({}), suggest upgrading QMM.\n", file, this->plugininfo->pifv_minor, QMM_PIFV_MINOR).c_str());
		return 0;
	}
	// if the plugin's minor version is lower, load, but suggest to upgrade plugin anyway
	if (this->plugininfo->pifv_minor < QMM_PIFV_MINOR)
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] WARNING: CPlugin::LoadQuery(\"{}\"): Plugin's minor interface version ({}) is less than QMM's ({}), suggest upgrading plugin.\n", file, this->plugininfo->pifv_minor, QMM_PIFV_MINOR).c_str());

	// find remaining neccesary functions or fail
	if ((this->QMM_Attach = (plugin_attach)this->dll.GetProc("QMM_Attach")) == NULL) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Unable to find \"QMM_Attach\" function in plugin\n", file).c_str());
		return 0;
	}
	if ((this->QMM_Detach = (plugin_detach)this->dll.GetProc("QMM_Detach")) == NULL) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Unable to find \"QMM_Detach\" function in plugin\n", file).c_str());
		return 0;
	}
	if ((this->QMM_vmMain = (plugin_vmmain)this->dll.GetProc("QMM_vmMain")) == NULL) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Unable to find \"QMM_vmMain\" function in plugin\n", file).c_str());
		return 0;
	}
	if ((this->QMM_syscall = (plugin_syscall)this->dll.GetProc("QMM_syscall")) == NULL) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Unable to find \"QMM_syscall\" function in plugin\n", file).c_str());
		return 0;
	}
	if ((this->QMM_vmMain_Post = (plugin_vmmain)this->dll.GetProc("QMM_vmMain_Post")) == NULL) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Unable to find \"QMM_vmMain_Post\" function in plugin\n", file).c_str());
		return 0;
	}
	if ((this->QMM_syscall_Post = (plugin_syscall)this->dll.GetProc("QMM_syscall_Post")) == NULL) {
		ENG_SYSCALL(QMM_ENG_MSG[QMM_G_PRINT], fmt::format("[QMM] ERROR: CPlugin::LoadQuery(\"{}\"): Unable to find \"QMM_syscall_Post\" function in plugin\n", file).c_str());
		return 0;
	}
