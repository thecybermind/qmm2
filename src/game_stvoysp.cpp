...
struct STVOYSP_GameSupport : public GameSupport {
    ...
    virtual const char* DefaultDLLName() { return "efgame" MOD_DLL; }
    virtual const char* DefaultModDir() { return "."; }
    virtual const char* GameName() { return "Star Trek Voyager: Elite Force (SP)"; }
    virtual const char* GameCode() { return "STVOYSP"; }
...
GEN_GAME_OBJ(STVOYSP);


// auto-detection logic for STVOYSP
bool STVOYSP_GameSupport::AutoDetect(APIType engineapi) {
    ...
    if (!Util::str_stristr(QMM::exe_file, "stvoy"))
        return false;
...
intptr_t STVOYSP_GameSupport::syscall_args(intptr_t cmd, intptr_t* args) {
    if (cmd != G_PRINT)
        QMMLOG(QMM_LOG_TRACE, "QMM") << "STVOYSP_GameSupport::syscall(" << EngMsgName(cmd) << "(" << cmd << ")) called\n";
    ...
    if (cmd != G_PRINT)
        QMMLOG(QMM_LOG_TRACE, "QMM") << "STVOYSP_GameSupport::syscall(" << EngMsgName(cmd) << "(" << cmd << ")) reutrning " << ret << "\n";
...
intptr_t STVOYSP_GameSupport::vmMain_args(intptr_t cmd, intptr_t* args) {
    QMMLOG(QMM_LOG_TRACE, "QMM") << "STVOYSP_GameSupport::vmMain(" << ModMsgName(cmd) << "(" << cmd << ")) called\n";
    ...
    QMMLOG(QMM_LOG_TRACE, "QMM") << "STVOYSP_GameSupport::vmMain(" << ModMsgName(cmd) << "(" << cmd << ")) returning " << ret << "\n";

    return ret;
}


void* STVOYSP_GameSupport::Entry(void* import, void*, APIType) {
    QMMLOG(QMM_LOG_DEBUG, "QMM") << "STVOYSP_GameSupport::Entry(" << import << ") called\n";
    ...
    QMMLOG(QMM_LOG_DEBUG, "QMM") << "STVOYSP_GameSupport::Entry(" << import << ") returning " << &qmm_export << "\n";
...
bool STVOYSP_GameSupport::ModLoad(void* entry, APIType mod_api) {
...
void STVOYSP_GameSupport::ModUnload() {
    orig_export = nullptr;
}


const char* STVOYSP_GameSupport::EngMsgName(intptr_t cmd) {
    switch (cmd) {
        GEN_CASE(G_PRINTF);
        GEN_CASE(G_WRITECAM);
        GEN_CASE(G_ERROR);
...
const char* STVOYSP_GameSupport::ModMsgName(intptr_t cmd) {
    switch (cmd) {
...
void STVOYSP_GameSupport::update_exports() {
    if (!orig_export)
        return;
...
game_import_t STVOYSP_GameSupport::orig_import;


game_export_t* STVOYSP_GameSupport::orig_export = nullptr;


game_import_t STVOYSP_GameSupport::qmm_import = {
...
std::vector<std::string> STVOYSP_GameSupport::entity_tokens;
size_t STVOYSP_GameSupport::token_counter = 0;
void STVOYSP_GameSupport::Init(const char* mapname, const char* spawntarget, int checkSum, const char* entstring, int levelTime, int randomSeed, int globalTime, SavedGameJustLoaded_e eSavedGameJustLoaded, qboolean qbLoadTransition) {
    if (entstring) {
...
// struct with lambdas that call QMM's vmMain function. this is given to the game engine
game_export_t STVOYSP_GameSupport::qmm_export = {
    GAME_API_VERSION,	// apiversion
    STVOYSP_GameSupport::Init,
