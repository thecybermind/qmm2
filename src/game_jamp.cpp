...
static game_import_t qmm_import = {
    ...
    GEN_IMPORT(SV_RegisterSharedMemory, G_SV_REGISTER_SHARED_MEMORY),
...
static intptr_t JAMP_syscall_GGA(intptr_t cmd, ...) {
    ...
    switch (cmd) {
        ...
        ROUTE_IMPORT(SV_RegisterSharedMemory, G_SV_REGISTER_SHARED_MEMORY);
...
static const char* JAMP_eng_msg_names(intptr_t cmd) {
    switch (cmd) {
        ...
        // OpenJK-only
        GEN_CASE(G_SV_REGISTER_SHARED_MEMORY);
