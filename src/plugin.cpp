...
namespace plugin {
	...
	bool plugin_load(plugin_t& p, std::string file) {
		...
		s_pluginvars.vmbase = mod::g_mod.vmbase;
		...
		if (!(p.QMM_Attach(g_gameinfo.pfnsyscall, mod::g_mod.pfnvmMain, &g_plugin_result, &s_pluginfuncs, &s_pluginvars))) {
